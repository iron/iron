...
//!
//! A [`Chain`] is a [`Handler`] that wraps another [`Handler`]. It is used to
//! attach middleware to the wrapped [`Handler`] using a `link` method
//! corresponding to each type of middleware. A sample middleware pipeline is
//! shown below:
//!
//! ```rust
//! use iron::prelude::*;
...
//! ```
//!
//! A request first travels through all [`BeforeMiddleware`], then a
//! [`Response`] is generated by the [`Handler`], which can be an arbitrary
//! nesting of [`AroundMiddleware`], then all [`AfterMiddleware`] are called
//! with both the [`Request`] and [`Response`]. After all [`AfterMiddleware`]
...
//! [`BeforeMiddleware`]: trait.BeforeMiddleware.html
//! [`AroundMiddleware`]: trait.AroundMiddleware.html
//! [`AfterMiddleware`]: trait.AfterMiddleware.html
//! [`Request`]: ../request/struct.Request.html
//! [`Response`]: ../response/struct.Response.html
//! [`IronError`]: ../error/struct.IronError.html
